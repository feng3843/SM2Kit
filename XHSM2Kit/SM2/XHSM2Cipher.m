...
@property(nonatomic, strong) XHECPointFp *g;

@property(nonatomic, assign) XHECCMode eccMode;

@end

@implementation XHSM2Cipher
                               ...
                               nHex:(NSString *)nHex
{
    if (self = [super init])
    {
        ...
        XHBigInt *p = [[XHBigInt alloc] initWithString:pHex radix:16];
        XHBigInt *a = [[XHBigInt alloc] initWithString:aHex radix:16];
        XHBigInt *b = [[XHBigInt alloc] initWithString:bHex radix:16];
        if ((gxHex.length != gyHex.length) || gyHex.length % 2 != 0)
        {
            NSAssert(NO, @"decode point error");
        }
        self.curve = [[XHECCurveFp alloc] initWithQ:p a:a b:b pointLen:gxHex.length * 2];
        self.g = [self.curve decodePointHex:[NSString stringWithFormat:@"04%@%@", gxHex, gyHex]];
        self.n = [[XHBigInt alloc] initWithString:nHex radix:16];
        
        self.eccMode = XHECCModeFp;
    }
    return self;
}
...
+ (XHSM2Cipher *)EC_Fp_SM2_256V1
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        param = [[XHSM2Cipher alloc] initWithFpParamPHex:pHex
                                                     aHex:aHex
                                                     bHex:bHex
                                                    gxHex:gxHex
                                                    gyHex:gyHex
...
{
    @autoreleasepool {
        XHBigInt *rng = [[XHBigInt alloc] initWithRandomBits:(int)[_n bitLength]];
        XHBigInt *d = [[rng modByBigInt:[_n subByBigInt:XHBigInt.one]] addByBigInt:XHBigInt.one];
        
        NSString *privateKey = [XHSMxUtils leftPad:[d toString:16] num:64];
        
        XHECPointFp *p = [_g multiply:d];
        NSString *px = [XHSMxUtils leftPad:[[[p getX] toBigInteger] toString:16] num:64];
        NSString *py = [XHSMxUtils leftPad:[[[p getY] toBigInteger] toString:16] num:64];
...
{
    @autoreleasepool {
        XHECPointFp *kGPoint = [_g multiply:k];
        return kGPoint;
    }
}
...
{
    @autoreleasepool {
        XHECPointFp *pPoint = [self.curve decodePointHex:[NSString stringWithFormat:@"04%@", pPointHex]];
        XHECPointFp *kPPoint = [pPoint multiply:k];
        return kPPoint;
    }
}
